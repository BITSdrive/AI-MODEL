...
def crop_face_from_id(cv_image):
    ...
    cropped_faces = RetinaFace.extract_faces(img=cv_image, align=False)
    if not cropped_faces:
        raise RuntimeError("[Error] Cannot find face in image.")
   
    cropped = cropped_faces[0]  # 첫 번째로 감지된 얼굴 이미지 사용
